...
Introduction: LM are trained by "predicting human text" but this is not really the ultimate goal of AI. "Predict the next word a human would write" versus "Generate the best quality output possible = totally different goals. What problems are caused by this- Models can make up facts (hallucinate, critical, factual error) and "word choices (minor errors), distributional shift- Model is trained on human written examples and then generates its OWN test-not human text- can cause problems. 

## What Problem Does RLHF Solve?
Improve model responses so they are not "predict a text", but rather quality generating models. Current problems being solved: Models can make up facts (hallucinate, critical, factual error) and "word choices" (minor errors), distributional shift- Model is trained on human written examples and then generates its OWN text-not human text- can cause problems. Problem: Model is in different situation than training. Solution: RLHF trains on model's OWN generations, so model learns in the right context. 

## How Reward Modeling Works
Collect human Preferences (Humans compare "is summary A or B better?) and build data set of preferences. Reward models learn "When humans prefer summary A over B, what made A better? Then model leanrs "what does 'good' look like. We used RL (reinforced Learning) to train the models. Use the reward model to score generated summaries, train the big model to maximize the reward score, use PPO algorithm. THEN REPEAT: Get Human Feedback, improve the reward model, improve the policy. 
...
## Questions I Have
How does PPO work? How do I know if the work we did for a client ACTUALLY improved the quality of their assistant responses? I don't necessarily get to see the end result of my work. What new issues arise after models are updated leading to new iterations? Are this consistent or random issues in AI?

## Answers so Questions Above
PPO = Proximal Policy Optimization- the RL algorithm they use to train models. EX: Reward Model says: "This output scores  8/10" Policy (Big Model) says: :"Let me try again to get a higher score". PPO says: "Keep trying, but don't change too much too fast" (can make things worse). Repeat this process until policy learns to maximize reward. Rubrics DEFINE reward. PPO uses REWARD to train model. Model GETS BETTER at what you rewarded it for. 
You are a piece of the loop and therefore cannot fully see the loop come to fruition. But confidence comes from High Quality Feedback through rubric design. Catch edge-cases (cause bias), high labeler agreement with fairness standards, interation visibility (staying on a project through multiple versions allows insgight into model improvement as we identify and address failures). Data was used by client for model improvement. 
Issues that arise from new iteratins include ripple effects that can come from creating rubrics to handle one situation, can causes effects on another situation and now it must ALSO be addressed in the rubrics. EX: Iteration 1- Model too formal; RUBRIC: "Use conversational tone". Iteration 2- Model is now too casual. PROBLEM: Less credibility in certain context. RUBRIC: "Match tone to context". THis is called "specification gaming" or "Goodhart's Law" = When a measure becomes a target, it seases to be a good measure". Patterns do emerge (Model's gaming metrics, distributinal shift, new edge cases emerge, fairness issues arise, tradeoff required) New issues also arise: Specific to task, data, or model architecture. 
...
```
## What went Wrong:
Researchers said "we want high-quality summaries" but when labelers evaluated- they got something different. Models looked good to labelers, but not researchers causing a "mismatch". Therefore, RLHF only works if: good feedback -> good reward model -> good policy. Quality = quality good or bad. 
